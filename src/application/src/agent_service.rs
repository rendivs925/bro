    ...
    pub async fn execute_agent(
        ...
        if let Some(semantic_memory) = &self.semantic_memory {
            if let Some(conversation_id) = &request.conversation_id {
                println!(
                    "🧠 Retrieving conversation history for: {}",
                    conversation_id
                );
        ...
        for i in 0..max_iters {
            ...
            if let Some(semantic_memory) = &self.semantic_memory {
                if let Some(conversation_id) = &request.conversation_id {
                    ...
                    {
                        println!(
                            "⚠️ Failed to store complete conversation in semantic memory: {}",
                            e
                        );
                    }
                }
            ...
            if let Some(semantic_memory) = &self.semantic_memory {
                if let Some(conversation_id) = &request.conversation_id {
                    ...
                    if let Some(message) = agent_context.conversation_history.last() {
                        ...
                        {
                            println!(
                                "⚠️ Failed to store conversation message in semantic memory: {}",
                                e
                            );
                        }
                    }
